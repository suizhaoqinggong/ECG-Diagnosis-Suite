...
export function AuthModal({ isOpen, onClose, defaultTab = 'login' }: AuthModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(tab === 'login' ? '邮箱或密码错误' : '注册失败，请重试')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-[24px] border border-[var(--border)] bg-[var(--bg)] p-8 shadow-[0_24px_48px_rgba(0,0,0,0.12)]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {tab === 'register' && (
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--ink-soft)]">
                显示名称（可选）
              </label>
              <input
                type="text"
